...
func TestListCategoriesSuccess(t *testing.T) {
	...
	assert.Equal(t, 301, int(responseBody["code"].(float64)))
	assert.Equal(t, "OK", responseBody["status"])
	...
	assert.Equal(t, category.Id, int(categoriesResponse1["id"].(float64)))
	assert.Equal(t, category.Name, categoriesResponse1["name"])
